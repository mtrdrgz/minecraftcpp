...
class RealmsPlayersTab extends GridLayoutTab implements RealmsConfigurationTab {
   ...
   RealmsPlayersTab(final RealmsConfigureWorldScreen configurationScreen, final Minecraft minecraft, final RealmsServer serverData) {
      ...
      RowHelper helper = this.layout.spacing(8).createRowHelper(1);
      this.invitedList = (RealmsPlayersTab.InvitedObjectSelectionList)helper.addChild(
   ...
   private abstract static class Entry extends net.minecraft.client.gui.components.ContainerObjectSelectionList.Entry<RealmsPlayersTab.Entry> {
   }
      ...
      public PlayerEntry(final PlayerInfo playerInfo) {
         ...
         this.removeButton = SpriteIconButton.builder(REMOVE_TEXT, button -> this.uninvite(index), false)
            ...
            .narration(
               defaultNarrationSupplier -> CommonComponents.joinForNarration(
                  new Component[]{
                     Component.translatable("mco.invited.player.narration", new Object[]{playerInfo.name}), (Component)defaultNarrationSupplier.get()
                  }
               )
            )
            .build();
