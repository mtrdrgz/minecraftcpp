// Byte-exact parity for net.minecraft.network.protocol.game.ClientboundGameRuleValuesPacket
// vs the REAL ClientboundGameRuleValuesPacket.STREAM_CODEC (tools/GameRuleValuesParity.java).
//
...
//   STREAM_CODEC = ByteBufCodecs.map(HashMap::new,
//                      ResourceKey.streamCodec(Registries.GAME_RULE),   // key
//                      ByteBufCodecs.STRING_UTF8)                       // value, max 32767
//                  .map(::new, ::values)
//        -> body only, NO packet-id prefix.
//   ResourceKey.streamCodec (ResourceKey.java:21) = Identifier.STREAM_CODEC.map(...)
//   Identifier.STREAM_CODEC  (Identifier.java:19)  = STRING_UTF8.map(parse, toString)
//        -> key encodes Utf8String.write( "namespace:path" ).
...
//   Utf8String.write (Utf8String.java:35-55):
...
//   writeVarInt(size) + per-entry writeUtf(keyIdString)+writeUtf(value) through
//   mc::net::PacketBuffer (PacketBuffer::writeString IS FriendlyByteBuf.writeUtf:
//   VarInt(byteLen)+UTF-8 bytes; STRING_UTF8 max length is 32767, the PacketBuffer default).
//
//   pkt_game_rule_values_cb_parity [--cases mcpp/build/pkt_game_rule_values.tsv]
//
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_game_rule_values.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        //     writeVarInt(size) then per entry writeUtf(keyIdString)+writeUtf(value).
        //     writeString(s, 32767) == FriendlyByteBuf.writeUtf: VarInt(byteLen)+UTF-8 bytes.
        PacketBuffer enc;
        enc.writeVarInt((int32_t)size);
        for (const auto& [k, v] : entries) {
            enc.writeString(k, 32767);
            enc.writeString(v, 32767);
        }
        ...
        if (ok) {
            for (int e = 0; e < size && ok; ++e) {
                std::string bk = dec.readString(32767);
                std::string bv = dec.readString(32767);
    ...
    std::cout << "PktGameRuleValuesParity cases=" << cases << " mismatches=" << mismatches << "\n";
