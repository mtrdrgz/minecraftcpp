// Byte-exact parity for net.minecraft.network.protocol.game.ClientboundPlayerPositionPacket
// vs the REAL ClientboundPlayerPositionPacket.STREAM_CODEC (tools/PktPlayerPositionParity.java).
//
...
//       Relative.SET_STREAM_CODEC,         ::relatives, -> int bitmask
//       ::new)
...
//                             -> output.writeInt(Relative.pack(set))  (BE 4 bytes).
//     pack ORs (1<<bit): X=0 Y=1 Z=2 Y_ROT=3 X_ROT=4 DELTA_X=5 DELTA_Y=6 DELTA_Z=7 ROTATE_DELTA=8.
//
// Full payload order:
//   VarInt(id) | px py pz (double,BE) | vx vy vz (double,BE) | yRot xRot (float,BE) | relativesMask (int,BE)
//
// This test exercises mc::net::PacketBuffer ONLY (no per-packet C++ class/header). It reconstructs
// the exact doubles/floats from their raw bits + the int mask, writes them through PacketBuffer in
// the EXACT codec order (writeVarInt / writeDouble x6 / writeFloat x2 / writeInt), requires the
...
//
//   pkt_player_position_parity [--cases mcpp/build/pkt_player_position.tsv]
//
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_player_position.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        std::string tag, name, idStr, pxStr, pyStr, pzStr, vxStr, vyStr, vzStr,
                    yrStr, xrStr, maskStr, nStr, expHex;
        ...
        if (!std::getline(ss, name, '\t') || !std::getline(ss, idStr, '\t') ||
            ...
            !std::getline(ss, maskStr, '\t') || !std::getline(ss, nStr, '\t') ||
        ...
        int32_t mask    = (int32_t)std::stoll(maskStr);
        ...
        enc.writeFloat(yRot);
        enc.writeFloat(xRot);
        // Relative set: packed bitmask as a big-endian int.
        enc.writeInt(mask);
    ...
    std::cout << "PktPlayerPositionParity cases=" << cases << " mismatches=" << mismatches << "\n";
