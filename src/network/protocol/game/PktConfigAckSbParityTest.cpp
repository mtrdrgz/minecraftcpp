// ServerboundConfigurationAcknowledgedPacket codec parity vs the REAL net.minecraft
// StreamCodec (tools/PktConfigAckSbParity.java ground truth).
//
// ServerboundConfigurationAcknowledgedPacket.STREAM_CODEC == StreamCodec.unit(INSTANCE):
// a unit codec whose encode writes NOTHING to the buffer and whose decode returns the
...
//
// TSV rows:  CONFIG_ACK_SB <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
//   pkt_config_ack_sb_parity [--cases mcpp/build/pkt_config_ack_sb.tsv]
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_config_ack_sb.tsv";
    ...
    std::cout << "PktConfigAckSbParity cases=" << cases << " mismatches=" << mismatches << "\n";
