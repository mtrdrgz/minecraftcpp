// ServerboundPlayerLoadedPacket codec parity vs the REAL net.minecraft StreamCodec
// (tools/PktPlayerLoadedSbParity.java ground truth).
//
// ServerboundPlayerLoadedPacket.STREAM_CODEC == StreamCodec.unit(new
// ServerboundPlayerLoadedPacket()): a unit codec whose encode writes NOTHING to the
// buffer and whose decode returns the unit instance without consuming any bytes. The
// packet is a zero-component record — no fields of any kind on the wire. So the encoded
// body is EXACTLY zero bytes (readableBytes()==0).
//
...
//
// TSV rows:  PLAYER_LOADED_SB <name> <readableBytes> <hexBytes(empty)> <isUnit>
//
//   pkt_player_loaded_sb_parity [--cases mcpp/build/pkt_player_loaded_sb.tsv]
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_player_loaded_sb.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        std::string tag, name, lenStr, expHex, unitStr;
        if (!std::getline(ss, tag, '\t')) continue;
        if (tag != "PLAYER_LOADED_SB") continue;
        ...
        if (!std::getline(ss, name, '\t') || !std::getline(ss, lenStr, '\t') ||
            !std::getline(ss, expHex, '\t') || !std::getline(ss, unitStr)) continue;
        ++cases;

        size_t expLen = (size_t)std::stoull(lenStr);
        int isUnit = (int)std::stoll(unitStr);

        bool ok = true;

        // The unit codec encodes the unit value by writing NOTHING. Mirror that: write
        ...
        // Sanity: Java's decode returned the unit value.
        if (isUnit != 1) {
            ok = false;
            std::cerr << "UNIT-MISMATCH name=" << name
                      << " java decode did not return the unit value\n";
        }
    ...
    std::cout << "PktPlayerLoadedSbParity cases=" << cases << " mismatches=" << mismatches << "\n";
