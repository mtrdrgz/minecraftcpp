// Byte-exact parity for net.minecraft.network.protocol.game.ServerboundChangeDifficultyPacket
// vs the REAL ServerboundChangeDifficultyPacket.STREAM_CODEC (tools/PktChangeDifficultySbParity.java).
//
// 26.1.2 wire format (verified against 26.1.2/src):
//   STREAM_CODEC = StreamCodec.composite(
//       Difficulty.STREAM_CODEC, ::difficulty,   -> VarInt(difficulty.getId())
//       ::new)
//   This packet carries EXACTLY ONE field: the Difficulty (no boolean, no trailing data).
...
// codec — writeVarInt(id) — and requires the produced bytes (as hex) AND the byte count == the Java
// ground truth, then decodes the Java bytes back through PacketBuffer requiring id round-trip exactly.
//
//   pkt_change_difficulty_sb_parity [--cases mcpp/build/pkt_change_difficulty_sb.tsv]
//
// Row: ENC <name> <id-dec> <readableBytes-dec> <hexBytes>
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_change_difficulty_sb.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        std::string tag, name, idStr, nStr, expHex;
        ...
        // (1) ENCODE: VarInt(getId()) through PacketBuffer; compare bytes + count.
        PacketBuffer enc;
        enc.writeVarInt(id);
        ...
        // (2) DECODE: pull id back out of the Java bytes; require exact round-trip.
    ...
    std::cout << "PktChangeDifficultySbParity cases=" << cases << " mismatches=" << mismatches << "\n";
