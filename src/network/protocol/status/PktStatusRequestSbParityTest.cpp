// ServerboundStatusRequestPacket codec parity vs the REAL net.minecraft StreamCodec
// (tools/PktStatusRequestSbParity.java ground truth).
//
// ServerboundStatusRequestPacket.STREAM_CODEC == StreamCodec.unit(INSTANCE): a unit
// codec whose encode writes NOTHING to the buffer and whose decode returns the
// singleton without consuming any bytes. The packet has no fields of any kind on the
// wire. So the encoded body is EXACTLY zero bytes (readableBytes()==0).
//
...
//
// TSV rows:  STATUS_REQUEST_SB <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
//   pkt_status_request_sb_parity [--cases mcpp/build/pkt_status_request_sb.tsv]
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_status_request_sb.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        if (tag != "STATUS_REQUEST_SB") continue;
    ...
    std::cout << "PktStatusRequestSbParity cases=" << cases << " mismatches=" << mismatches << "\n";
