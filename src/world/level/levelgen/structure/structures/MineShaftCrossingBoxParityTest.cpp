// Bit-exact parity gate for the RNG-driven MineShaftCrossing.findCrossing
// geometry, ported in
//   world/level/levelgen/structure/structures/MineShaftCrossingBox.h
...
//
// Ground truth (mcpp/tools/MineShaftCrossingBoxParity.java) calls the REAL
//   net.minecraft...structures.MineshaftPieces.MineShaftCrossing.findCrossing(
...
//
//   mine_shaft_crossing_box_parity --cases mcpp/build/mine_shaft_crossing_box.tsv

#include "world/level/levelgen/structure/structures/MineShaftCrossingBox.h"
...
int main(int argc, char** argv) {
    ...
    if (casesPath.empty()) {
        std::cerr << "usage: mine_shaft_crossing_box_parity --cases <tsv>\n";
        return 2;
    }
    ...
    while (std::getline(in, line)) {
        ...
        if (tag != "CROSS") { fail("UNKNOWN_TAG " + tag); continue; }
        ++checks;
        ...
        std::optional<BoundingBox> got =
            ms::findCrossing(*random, footX, footY, footZ, dir, neverCollides);
    ...
    std::cout << "MineShaftCrossing checks=" << checks << " mismatches=" << mism << "\n";
    return mism == 0 ? 0 : 1;
}
