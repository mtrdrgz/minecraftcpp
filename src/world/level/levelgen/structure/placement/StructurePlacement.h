...
struct StructurePlacement {
    ...
    // exclusion zones can still query their vanilla placement chunks, and expose a
    // separate placement-parity predicate below so tests still verify the grid.
...
public:
    ...
    // loaded for exclusion zones/parity, but return false here.
    static bool isSupported(const StructurePlacement& p) {
        return p.generationEnabled && isPlacementSupported(p);
    }
};
