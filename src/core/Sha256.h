...
// Portable SHA-256 (FIPS 180-4). Replaces the Windows-only BCrypt path so the
// biome seed obfuscation (and thus the whole worldgen) builds and runs on any
// platform. Verified against the known BiomeManager.obfuscateSeed values.
...
inline std::array<uint8_t, 32> sha256(const uint8_t* data, std::size_t len) {
