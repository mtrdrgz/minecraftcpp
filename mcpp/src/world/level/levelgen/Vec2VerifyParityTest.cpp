// Verification parity test for net.minecraft.world.phys.Vec2 (the existing certified
// engine header world/phys/Vec2.h). Ground truth: tools/Vec2VerifyParity.java.
// Bit-exact: every float is compared by its raw IEEE-754 bits (std::bit_cast),
// never by value. This is a second, finite/physical battery that independently
// re-certifies the engine header — it #includes and exercises the real header,
// it does not duplicate or redefine any of its logic.
//
//   vec2_verify_parity --cases mcpp/build/vec2_verify.tsv
...
int main(int argc, char** argv) {
    ...
    if (casesPath.empty()) { std::cerr << "usage: vec2_verify_parity --cases <tsv>\n"; return 2; }
    ...
    std::cout << "Vec2Verify cases=" << total << " mismatches=" << mism << "\n";
    return mism == 0 ? 0 : 1;
}
