...
struct StructurePlacement {
    ...
    // exclusion zones can still query their vanilla placement chunks, and so the
    // placement parity gate still verifies the vanilla structure chunk grid.
...
public:
    ...
    // True when the placement can both be evaluated and generated by the current
    // runtime. Known-broken families stay loaded but return false here.
    static bool isGenerationSupported(const StructurePlacement& p) {
        return p.generationEnabled && isSupported(p);
    }
};
