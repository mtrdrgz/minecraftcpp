...
//   (26.1.2).
//
// Each NetherFortressPieces.<X>.createPiece(...) builds its candidate box via
// BoundingBox.orientBox(footX,footY,footZ, <type consts>, direction) and accepts
// it iff NetherBridgePiece.isOkBox(box) (== box.minY() > 10). Ground truth:
// tools/NetherFortressPieceBoxParity.java drives the REAL BoundingBox.orientBox
// with each type's constants and the REAL (reflected) isOkBox, and emits a TSV.
// We recompute the box + okBox from NetherFortressPieceBox.h and compare. All
// values are decimal ints / a 0|1 flag, so the gate is exact.
//
//   nether_fortress_piece_box_parity --cases mcpp/build/nether_fortress_piece_box.tsv
//
...
#include "NetherFortressPieceBox.h"
...
int main(int argc, char** argv) {
    ...
    if (casesPath.empty()) { std::cerr << "usage: nether_fortress_piece_box_parity --cases <tsv>\n"; return 2; }
    ...
    while (std::getline(in, line)) {
        ...
        auto type = static_cast<NetherFortressPiece>(typeIdx);
        NetherFortressPieceResult r =
            netherFortressCreatePieceGeometry(type, footX, footY, footZ, dirFromOrdinal(dirOrd));
    ...
    std::cout << "NetherFortressPieceBox checks=" << total << " mismatches=" << mism << "\n";
    return mism == 0 ? 0 : 1;
}
