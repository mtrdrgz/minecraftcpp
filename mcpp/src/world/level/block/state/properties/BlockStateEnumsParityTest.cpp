// Parity test for the per-constant ordinal + getSerializedName() tables of the
// StringRepresentable block-state-property enums. Ground truth:
//   tools/BlockStateEnumsParity.java.
//
// For every enum, the GT emits a COUNT row and one NAME row per constant. We
// rebuild the C++ table from BlockStateEnums.h and compare:
...
//
//   attach_face_props_parity --cases mcpp/build/attach_face_props.tsv

#include "BlockStateEnums.h"
...
const bsp::BlockStateEnumDesc* findDesc(const std::string& javaName) {
    for (const bsp::BlockStateEnumDesc* d : bsp::allBlockStateEnums())
...
int main(int argc, char** argv) {
    ...
    if (casesPath.empty()) {
        std::cerr << "usage: attach_face_props_parity --cases <tsv>\n";
        return 2;
    }
    ...
    while (std::getline(in, line)) {
        ...
        if (tag == "COUNT") {
            ...
            const bsp::BlockStateEnumDesc* d = findDesc(javaName);
            ...
            if (bsp::getSerializedName(javaName, ordinal) != expected)
                fail(line, "getSerializedName helper");
        } else {
            ++total;
    ...
    std::cout << "BlockStateEnums cases=" << total << " mismatches=" << mism << "\n";
    return mism == 0 ? 0 : 1;
}
