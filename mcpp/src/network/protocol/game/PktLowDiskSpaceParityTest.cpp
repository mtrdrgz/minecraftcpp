// ClientboundLowDiskSpaceWarningPacket codec parity vs the REAL net.minecraft StreamCodec
// (tools/PktLowDiskSpaceParity.java ground truth).
//
// ClientboundLowDiskSpaceWarningPacket.STREAM_CODEC == StreamCodec.unit(INSTANCE): a unit
// codec whose encode writes NOTHING to the buffer and whose decode returns the singleton
// without consuming any bytes. The packet has no fields of any kind on the wire. So the
// encoded body is EXACTLY zero bytes (readableBytes()==0).
//
...
//
// TSV rows:  LOW_DISK_SPACE <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
//   pkt_low_disk_space_parity [--cases mcpp/build/pkt_low_disk_space.tsv]
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_low_disk_space.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        if (tag != "LOW_DISK_SPACE") continue;
    ...
    std::cout << "PktLowDiskSpaceParity cases=" << cases << " mismatches=" << mismatches << "\n";
