// Byte-exact parity for net.minecraft.network.protocol.game.ClientboundTeleportEntityPacket
// vs the REAL ClientboundTeleportEntityPacket.STREAM_CODEC (tools/PktTeleportEntityParity.java).
//
...
//       PositionMoveRotation.STREAM_CODEC, ::change,    -> nested record
//       Relative.SET_STREAM_CODEC,         ::relatives, -> plain int bitmask (see below)
...
//       -> ByteBufCodecs.INT.encode = output.writeInt(pack(set)): a PLAIN 4-byte big-endian
//          int (NOT a VarInt). pack(set) ORs (1 << bit) per present Relative.
//
// Full payload order:
//   VarInt(id) | px py pz (double,BE) | vx vy vz (double,BE) | yRot xRot (float,BE)
//             | relMask (int,BE) | onGround (byte)
//
...
// order, requires the produced bytes (as hex) AND the byte count == the Java ground truth, then
// decodes the Java bytes back through PacketBuffer and requires every field round-trips bit-for-bit.
//
//   pkt_teleport_entity_parity [--cases mcpp/build/pkt_teleport_entity.tsv]
//
...
int main(int argc, char** argv) {
    ...
    while (std::getline(f, line)) {
        ...
        std::string tag, name, idStr, pxStr, pyStr, pzStr, vxStr, vyStr, vzStr,
                    yrStr, xrStr, relStr, ogStr, nStr, expHex;
        ...
        if (!std::getline(ss, name, '\t') || !std::getline(ss, idStr, '\t') ||
            ...
            !std::getline(ss, relStr, '\t') || !std::getline(ss, ogStr, '\t') ||
        ...
        int32_t relMask = (int32_t)std::stoll(relStr);
        ...
        enc.writeFloat(yRot);
        enc.writeFloat(xRot);
        // Relative.SET_STREAM_CODEC -> ByteBufCodecs.INT -> plain big-endian writeInt(pack).
        enc.writeInt(relMask);
    ...
    std::cout << "PktTeleportEntityParity cases=" << cases << " mismatches=" << mismatches << "\n";
