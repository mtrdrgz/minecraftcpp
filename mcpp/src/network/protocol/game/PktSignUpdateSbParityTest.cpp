// Byte-exact parity gate for net.minecraft.network.protocol.game.ServerboundSignUpdatePacket
// against the REAL STREAM_CODEC (tools/PktSignUpdateSbParity.java ground truth).
//
// The packet body is exactly (ServerboundSignUpdatePacket.java:36-43):
...
//           for i in 0..3: output.writeUtf(lines[i]); // VarInt byte-len + UTF-8
//   read  : this.pos         = input.readBlockPos();   == BlockPos.of(input.readLong())
//           this.isFrontText = input.readBoolean();
//           for i in 0..3: lines[i] = input.readUtf(384);
...
//   writeLong(asLong(x,y,z)) + writeBool(isFrontText) + 4x writeString(line)
// is byte-for-byte the same as the real codec, and readLong()/readBool()/readString()
// round-trips the fields value-for-value. writeString == writeUtf (Utf8String.write:
// VarInt byte-length prefix + UTF-8 bytes). The four lines arrive as UTF-8 HEX columns.
//
//   pkt_sign_update_sb_parity [--cases mcpp/build/pkt_sign_update_sb.tsv]
...
int main(int argc, char** argv) {
    std::string casesPath = "mcpp/build/pkt_sign_update_sb.tsv";
    ...
    while (std::getline(f, line)) {
        ...
        // ENC <name> <x> <y> <z> <isFrontText> <l0hex> <l1hex> <l2hex> <l3hex> <readableBytes> <hexBytes>
        std::string name, xS, yS, zS, fS, l0H, l1H, l2H, l3H, rbS, expHex;
        if (!std::getline(ss, name, '\t') || !std::getline(ss, xS, '\t')
            || !std::getline(ss, yS, '\t') || !std::getline(ss, zS, '\t')
            || !std::getline(ss, fS, '\t') || !std::getline(ss, l0H, '\t')
            || !std::getline(ss, l1H, '\t') || !std::getline(ss, l2H, '\t')
            || !std::getline(ss, l3H, '\t') || !std::getline(ss, rbS, '\t')
        ...
        std::string l0 = hexToStr(l0H), l1 = hexToStr(l1H),
                    l2 = hexToStr(l2H), l3 = hexToStr(l3H);
        size_t expReadable = (size_t)std::stoull(rbS);

        bool ok = true;

        // write(): writeBlockPos(pos)=writeLong(asLong) + writeBoolean + 4x writeUtf.
        PacketBuffer enc;
        try {
        ...
        try {
            ...
            std::string g0 = dec.readString(384);
            std::string g1 = dec.readString(384);
            std::string g2 = dec.readString(384);
            std::string g3 = dec.readString(384);
            ...
            if (g0 != l0 || g1 != l1 || g2 != l2 || g3 != l3) {
                std::cerr << "DEC-LINES " << name << " mismatch\n";
                ok = false;
            }
            if (dec.remaining() != 0) {
    ...
    std::cout << "PktSignUpdateSbParity cases=" << cases
