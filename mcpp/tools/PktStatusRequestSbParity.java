// Ground truth for the ServerboundStatusRequestPacket codec.
//
...
//   STATUS_REQUEST_SB <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
// where <hexBytes> is the empty string for a zero-length body. The C++
// pkt_status_request_sb_parity asserts the same: an empty PacketBuffer (nothing
...
public class PktStatusRequestSbParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StreamCodec<ByteBuf, ServerboundStatusRequestPacket> codec =
        ...
        for (String name : names) {
            ...
            if (name.equals("encode_twice_same_buf")) {
                ...
                codec.encode(buf, ServerboundStatusRequestPacket.INSTANCE);
                codec.encode(buf, ServerboundStatusRequestPacket.INSTANCE);
            } else {
                codec.encode(buf, ServerboundStatusRequestPacket.INSTANCE);
            }
            ...
            // consume no bytes.
            ServerboundStatusRequestPacket back = codec.decode(buf);
            int isInstance = (back == ServerboundStatusRequestPacket.INSTANCE) ? 1 : 0;

            // hex column is "" for a zero-byte body; we print it explicitly so the TSV
            // column is always present.
            O.printf("STATUS_REQUEST_SB\t%s\t%d\t%s\t%d%n",
