// Ground truth for the ClientboundLowDiskSpaceWarningPacket codec.
//
...
//       new ClientboundLowDiskSpaceWarningPacket();
//   public static final StreamCodec<ByteBuf, ClientboundLowDiskSpaceWarningPacket>
...
//   LOW_DISK_SPACE <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
// where <hexBytes> is the empty string for a zero-length body. The C++
// pkt_low_disk_space_parity asserts the same: an empty PacketBuffer (nothing
...
public class PktLowDiskSpaceParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StreamCodec<ByteBuf, ClientboundLowDiskSpaceWarningPacket> codec =
            ClientboundLowDiskSpaceWarningPacket.STREAM_CODEC;
        ...
        for (String name : names) {
            ...
            if (name.equals("encode_twice_same_buf")) {
                ...
                codec.encode(buf, ClientboundLowDiskSpaceWarningPacket.INSTANCE);
                codec.encode(buf, ClientboundLowDiskSpaceWarningPacket.INSTANCE);
            } else {
                codec.encode(buf, ClientboundLowDiskSpaceWarningPacket.INSTANCE);
            }
            ...
            // consume no bytes.
            ClientboundLowDiskSpaceWarningPacket back = codec.decode(buf);
            int isInstance = (back == ClientboundLowDiskSpaceWarningPacket.INSTANCE) ? 1 : 0;

            // hex column is "" for a zero-byte body; we print it explicitly so the TSV
            // column is always present.
            O.printf("LOW_DISK_SPACE\t%s\t%d\t%s\t%d%n",
