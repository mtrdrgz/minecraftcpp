// Ground truth for the ClientboundPlayerCombatEnterPacket codec.
//
// net.minecraft.network.protocol.game.ClientboundPlayerCombatEnterPacket
// (26.1.2/src/.../ClientboundPlayerCombatEnterPacket.java):
//
//   public static final ClientboundPlayerCombatEnterPacket INSTANCE =
//       new ClientboundPlayerCombatEnterPacket();
//   public static final StreamCodec<ByteBuf, ClientboundPlayerCombatEnterPacket> STREAM_CODEC =
//       StreamCodec.unit(INSTANCE);
//   private ClientboundPlayerCombatEnterPacket() {}   // private no-arg ctor, singleton
//
// StreamCodec.unit(instance).encode(output, value) writes NOTHING to the buffer —
// it only asserts value.equals(instance) (the singleton) and otherwise throws
// IllegalStateException (StreamCodec.java lines 49-63). Its decode(input) returns the
// singleton without consuming any bytes. Therefore the encoded body is EXACTLY zero
// bytes (readableBytes()==0), and decode yields the INSTANCE. There are NO fields of
// any kind on the wire — no registry-held types, no ItemStack, no Component, no NBT,
// no Holder — nothing for the C++ PacketBuffer to build.
//
...
// pkt_player_combat_enter_parity asserts the same: an empty PacketBuffer (nothing
...
public class PktPlayerCombatEnterParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StreamCodec<ByteBuf, ClientboundPlayerCombatEnterPacket> codec =
            ClientboundPlayerCombatEnterPacket.STREAM_CODEC;
        ...
        for (String name : names) {
            ...
            if (name.equals("encode_twice_same_buf")) {
                ...
                codec.encode(buf, ClientboundPlayerCombatEnterPacket.INSTANCE);
                codec.encode(buf, ClientboundPlayerCombatEnterPacket.INSTANCE);
            } else {
                codec.encode(buf, ClientboundPlayerCombatEnterPacket.INSTANCE);
            }
            ...
            // consume no bytes.
            ClientboundPlayerCombatEnterPacket back = codec.decode(buf);
            int isInstance = (back == ClientboundPlayerCombatEnterPacket.INSTANCE) ? 1 : 0;
