// Ground truth for net.minecraft.network.protocol.configuration.ServerboundSelectKnownPacks's
// StreamCodec.
//
// The packet is a record(List<KnownPack> knownPacks)
// (ServerboundSelectKnownPacks.java:11-14). Its STREAM_CODEC is StreamCodec.composite over,
// in this exact wire order:
//   KnownPack.STREAM_CODEC.apply(ByteBufCodecs.list(64)) -> knownPacks
// ByteBufCodecs.list(64) is ByteBufCodecs.collection(ArrayList::new, KnownPack.STREAM_CODEC, 64)
// (ByteBufCodecs.java:413-435): it writes writeCount(size) = VarInt(size) (ByteBufCodecs.java:399-405),
// then encodes each element in order.
...
// A DEC line mirrors ENC after a decode round-trip sanity check (abort on any drift).
...
public class PktSelectKnownPacksSbParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StreamCodec<ByteBuf, ServerboundSelectKnownPacks> CODEC =
            (StreamCodec<ByteBuf, ServerboundSelectKnownPacks>)
        ...
        // Max allowed list size (64 packs) to stress the list VarInt and element loop.
        List<KnownPack> big = new ArrayList<>();
        for (int i = 0; i < 64; i++) big.add(new KnownPack("mod" + i, "id" + i, "v" + i));
        emit(CODEC, big);
    }

    static void emit(StreamCodec<ByteBuf, ServerboundSelectKnownPacks> CODEC, List<KnownPack> packs) {
        ServerboundSelectKnownPacks pkt = new ServerboundSelectKnownPacks(packs);
        ...
        ServerboundSelectKnownPacks dec = CODEC.decode(rbuf);
