// Ground truth for the ServerboundConfigurationAcknowledgedPacket codec.
//
...
//       new ServerboundConfigurationAcknowledgedPacket();
//   public static final StreamCodec<ByteBuf, ServerboundConfigurationAcknowledgedPacket>
//       STREAM_CODEC = StreamCodec.unit(INSTANCE);
//
// StreamCodec.unit(instance).encode(output, value) writes NOTHING to the buffer — it
// only asserts value.equals(instance) (the singleton) and otherwise throws. Its
...
// INSTANCE. There are no fields of any kind on the wire (no registry-held types, no
// ItemStack, no Component, no NBT, no Holder, no SoundEvent) — nothing for
// PacketBuffer to build. (The packet is also isTerminal()==true, but terminality is a
// listener-handling property, not a wire field.)
//
// We drive the REAL StreamCodec (encode + round-trip decode) and dump:
//   CONFIG_ACK_SB <name> <readableBytes> <hexBytes(empty)> <isInstance>
//
// where <hexBytes> is the empty string for a zero-length body. The C++
// pkt_config_ack_sb_parity asserts the same: an empty PacketBuffer (nothing written)
// with size()==0 and hex=="" matching the expected zero-byte encoding.
...
public class PktConfigAckSbParity {
    ...
    public static void main(String[] args) throws Exception {
        ...
        StreamCodec<ByteBuf, ServerboundConfigurationAcknowledgedPacket> codec =
            ServerboundConfigurationAcknowledgedPacket.STREAM_CODEC;
        ...
        for (String name : names) {
            ...
            if (name.equals("encode_twice_same_buf")) {
                ...
                codec.encode(buf, ServerboundConfigurationAcknowledgedPacket.INSTANCE);
                codec.encode(buf, ServerboundConfigurationAcknowledgedPacket.INSTANCE);
            } else {
                codec.encode(buf, ServerboundConfigurationAcknowledgedPacket.INSTANCE);
            }
            ...
            // consume no bytes.
            ServerboundConfigurationAcknowledgedPacket back = codec.decode(buf);
            int isInstance = (back == ServerboundConfigurationAcknowledgedPacket.INSTANCE) ? 1 : 0;

            // hex column is "" for a zero-byte body; we print it explicitly so the TSV
            // column is always present.
            O.printf("CONFIG_ACK_SB\t%s\t%d\t%s\t%d%n",
